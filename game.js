(() => {
  ...
  playerSprite.image.onload = () => {
    playerSprite.sheet = makeTransparentSheet(playerSprite.image);
  ...
  function currentPlayerFrame(p) {
    if (!p.onGround) {
      return p.vy < 0 ? playerSprite.frames.air[0] : playerSprite.frames.air[1];
    }
    if (Math.abs(p.vx) > 30) {
      return playerSprite.frames.run[Math.floor(performance.now() / 90) % 4];
    }
    return playerSprite.frames.idle[Math.floor(performance.now() / 240) % 4];
  }
  ...
  function prepareSpriteFrames(sprite, onReady = null) {
    ...
    for (const [stateName, frames] of Object.entries(sprite.frames)) {
      ...
      frames.forEach((frame, index) => {
        jobs.push(() => {
          prepared[stateName][index] = makeTransparentFrame(sprite.image, frame);
        });
      });
    }
  ...
  function makeTransparentFrame(image, frame) {
    ...
    const imageData = frameCtx.getImageData(0, 0, frame.w, frame.h);
    clearEdgeBackground(imageData, frame.w, frame.h);
    frameCtx.putImageData(imageData, 0, 0);
    return { canvas: frameCanvas, w: frame.w, h: frame.h };
  }
  ...
  function updateHud() {
    if (!state) return;
    const p = state.player;
    hud.mangoes.textContent = `${p.mangoes} / ${WIN_MANGOES}`;
    hud.timer.textContent = formatTime(state.timeLeft);
    hud.stick.textContent = `Stick: ${p.hasStick ? "Yes" : "No"}`;
    hud.stones.textContent = `Stones: ${p.stones}`;
